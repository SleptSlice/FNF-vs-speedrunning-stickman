...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['Stickin!!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	var dialogue:Array<String> = ['blah blah blah', 'poop'];
...
//		debugKeysCharacter = ClientPrefs.copyKey(ClientPrefs.keyBinds.get('debug_2'));
		...
		if(SONG.stage == null || SONG.stage.length < 1) {
			switch (songName)
			{
				case 'speed':
					curStage = 'grassLand';
				case 'running':
					curStage = 'grassLandSunset';
				case 'running':
					curStage = 'grassLandNight';
		...
		switch (curStage)
		{
			case 'grassLand': //Week Stickman
				var grassSky:BGSprite = new BGSprite('GrassSky', -400, 0, 0.9, 0.9);
				grassSky.setGraphicSize(Std.int(grassSky.width * 1.8));
				grassSky.updateHitbox();
				add(grassSky);

				var grassLand:BGSprite = new BGSprite('GrassLand', -360, 600, 0.9, 0.9);
				grassLand.setGraphicSize(Std.int(grassLand.width * 1.8));
				grassLand.updateHitbox();
				add(grassLand);

				var grassSun = new BGSprite('GrassSun', 0, 0, ['GrassSun']);
				add(grassSun);

				case 'grassLandSunset': //Week Stickman
				var grassSky:BGSprite = new BGSprite('sunset/GrassSky', -400, 0, 0.9, 0.9);
				grassSky.setGraphicSize(Std.int(grassSky.width * 1.8));
				grassSky.updateHitbox();
				add(grassSky);

				var grassLand:BGSprite = new BGSprite('sunset/GrassLand', -360, 600, 0.9, 0.9);
				grassLand.setGraphicSize(Std.int(grassLand.width * 1.8));
				grassLand.updateHitbox();
				add(grassLand);

				case 'grassLandNight': //Week Stickman
				var grassSky:BGSprite = new BGSprite('night/GrassSky', -400, 0, 0.9, 0.9);
				grassSky.setGraphicSize(Std.int(grassSky.width * 1.8));
				grassSky.updateHitbox();
				add(grassSky);

				var grassLand:BGSprite = new BGSprite('night/GrassLand', -360, 600, 0.9, 0.9);
				grassLand.setGraphicSize(Std.int(grassLand.width * 1.8));
				grassLand.updateHitbox();
				add(grassLand);
